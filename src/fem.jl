...
#
function solveq!(x,Kp,f,bcdofs_in,bcval_in)
    nd          = size(K,1)
    pdofs_in    = bcdofs_in
    fdofs_in    = setdiff(1:nd,pdofs_in)

    prob = LinearProblem(Kp[fdofs_in,fdofs_in], f[fdofs_in] - Kp[fdofs_in,pdofs_in]*bcval_in)
    ...
    #x[fdofs_in] = solve(prob, KrylovJL_GMRES(), Pl = pl).u
