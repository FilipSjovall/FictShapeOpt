...
global outit     = 0;
global change    = 1;
global xmax      = 0.5*ones(n_mma);
global xmin      =-0.5*ones(n_mma);

m_indices = findall(x -> x in collect(nₘ), free_d)
s_indices = findall(x -> x in collect(nₛ), free_d)

global xmax[m_indices].=   0.5
global xmin[m_indices].=  -0.5
global xmax[s_indices].=   0.5
global xmin[s_indices].=  -0.5
