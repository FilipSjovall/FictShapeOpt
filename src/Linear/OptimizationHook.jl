...
# Create grids  #
# # # # # # # # #
grid1 = createBoxMeshRev("box_1", xₗ, yₗ, Δx, Δy, 0.1)
grid2 = createBoxMeshRev("box_2", xᵤ, yᵤ, Δx, Δy, 0.1)
...
bcdofs_opt       = [bcdof_left; bcdof_top; bcdof_right];
...
function Optimize(dh)
    # Flytta allt nedan till init_opt?
        ...
        global T[bcdof_top[bcdof_top .% 2 .==0]] .= 1.0
        g₁             = 0.0
        g₂             = 0.0
    #
    while kktnorm > tol || OptIter < 200
        ...
        p2 = plot(1:true_iteration,[v_hist[1:true_iteration],p_hist[1:true_iteration]],label = ["Volume Constraint" "Uniform pressure Constraint" "Objective"])
        display(p2)
