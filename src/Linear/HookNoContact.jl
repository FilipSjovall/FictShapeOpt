...
# ------------------------------------------- #
addfaceset!(dh.grid, "Γ_left", x -> (x[1]≈th && x[2] ≥ th*1.0))
Γ_left = getfaceset(dh.grid, "Γ_left")
#
addnodeset!(dh.grid, "n_left", x -> (x[1] ≈ th && x[2] ≥ th*1.0))
...
global F_tar = [-0.02, -0.04, -0.06, -0.08, -0.1, -0.12, -0.14, -0.16, -0.18, -0.20] .* 25
global F_tar[4:end-1] .= F_tar[4]
...
function Optimize(dh)
    ...
    while kktnorm > tol || OptIter < 2
        ...
        a, _, Fₑₓₜ, Fᵢₙₜ, K, a_hist = solver_hook(dh, coord, Δ, nloadsteps)
